...
class ReportValidator:
    ...
    def validate(self) -> bool:
        ...
        checks = [
            ...
            ("Claims-Evidence Table", self._check_claims_evidence_section),
            ("Competing Hypotheses", self._check_competing_hypotheses_section),
            ("Citations", self._check_citations),
            ("Citation Density", self._check_citation_density),
            ("Bibliography", self._check_bibliography),
            ("Marketing Language", self._check_marketing_language),
            ("Vague Attribution", self._check_vague_attribution),
            ("Prose vs Bullets Ratio", self._check_prose_bullets_ratio),
    ...
    def _check_claims_evidence_section(self) -> bool:
        """Check that a Claims-Evidence Table section is present"""
        pattern = r'^##\s+.*(?:Claims[-\s]Evidence|Claims)'
        if re.search(pattern, self.content, re.MULTILINE | re.IGNORECASE):
            return True
        self.errors.append("Missing required 'Claims-Evidence Table' section (expected ## header containing 'Claims-Evidence' or 'Claims')")
        return False

    def _check_competing_hypotheses_section(self) -> bool:
        """Check that a Competing Hypotheses section is present"""
        pattern = r'^##\s+.*(?:Competing\s+Hypotheses|Hypotheses)'
        if re.search(pattern, self.content, re.MULTILINE | re.IGNORECASE):
            return True
        self.errors.append("Missing required 'Competing Hypotheses' section (expected ## header containing 'Competing Hypotheses' or 'Hypotheses')")
        return False
    ...
    def _check_citation_density(self) -> bool:
        """Check at least 1 citation [N] per 200 words of body text (exclude bibliography and methodology)"""
        # Extract body text: remove bibliography and methodology sections
        body = self.content

        # Remove bibliography section
        body = re.sub(r'## Bibliography.*', '', body, flags=re.DOTALL | re.IGNORECASE)
        # Remove methodology section
        body = re.sub(r'## Methodology.*?(?=##|\Z)', '', body, flags=re.DOTALL | re.IGNORECASE)
        ...
        expected_min = word_count // 200
        if expected_min < 1:
            expected_min = 1

        if citation_count < expected_min:
            self.warnings.append(
                f"Low citation density: {citation_count} citations in {word_count} body words "
                f"(expected at least 1 per 200 words = {expected_min})"
            )
    ...
    def _check_marketing_language(self) -> bool:
        """Flag superlatives without data backing (no citation within 50 chars)"""
        superlatives = [
            r'\bbest\b', r'\bleading\b', r'\brevolutionary\b',
            r'\bgame[-\s]changing\b', r'\bunprecedented\b', r'\bworld[-\s]class\b'
        ]

        flagged = []
        for sup_pattern in superlatives:
            for match in re.finditer(sup_pattern, self.content, re.IGNORECASE):
                start = match.start()
                end = match.end()
                word = match.group()
                # Check if there's a citation [N] within 50 chars after the match
                following_text = self.content[end:end + 50]
                if not re.search(r'\[\d+\]', following_text):
                    # Get context: surrounding text for the warning
                    ctx_start = max(0, start - 20)
                    ctx_end = min(len(self.content), end + 30)
                    context = self.content[ctx_start:ctx_end].replace('\n', ' ')
                    flagged.append(f'"{word}" without citation near: "...{context}..."')

        if flagged:
            # Limit to first 5 to avoid noise
            shown = flagged[:5]
            extra = f" (and {len(flagged) - 5} more)" if len(flagged) > 5 else ""
            self.warnings.append(
                f"Marketing language without citations ({len(flagged)} instances){extra}: "
                + "; ".join(shown)
            )

        return True  # Warnings only, not errors

    def _check_vague_attribution(self) -> bool:
        """Flag vague attributions that should cite specific sources"""
        vague_patterns = [
            (r'\bstudies show\b', 'studies show'),
            (r'\bresearch suggests\b', 'research suggests'),
            (r'\bexperts believe\b', 'experts believe'),
            (r'\bit is widely known\b', 'it is widely known'),
            (r'\bmany argue\b', 'many argue'),
        ]

        flagged = []
        for vague_re, label in vague_patterns:
            for match in re.finditer(vague_re, self.content, re.IGNORECASE):
                end = match.end()
                following_text = self.content[end:end + 50]
                if not re.search(r'\[\d+\]', following_text):
                    flagged.append(f'"{label}" without specific citation')

        if flagged:
            shown = flagged[:5]
            extra = f" (and {len(flagged) - 5} more)" if len(flagged) > 5 else ""
            self.warnings.append(
                f"Vague attributions without citations ({len(flagged)} instances){extra}: "
                + "; ".join(shown)
            )

        return True  # Warnings only, not errors

    def _check_prose_bullets_ratio(self) -> bool:
        """Warn if bullet lines exceed 20% of content lines"""
        lines = self.content.split('\n')
        content_lines = [l for l in lines if l.strip()]  # non-empty lines
        bullet_lines = [l for l in content_lines if re.match(r'^\s*[-*]\s', l)]
        ...
        if ratio > 0.20:
            ...
            self.warnings.append(
                f"High bullet-to-prose ratio: {pct}% of content lines are bullets "
                f"({bullet_count}/{total}). Consider converting lists to prose for better readability."
            )
...
Examples:
  ...
  python validate_report.py -r ~/data/skills/deep-research/research_report.md
        """
    )
