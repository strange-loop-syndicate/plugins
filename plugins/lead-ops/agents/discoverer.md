...
- Each entry in `candidates` is an object with required fields: `title` (string), `url` (string, absolute), `source_id` (string from source_id input), `source_type` (string), `source_specific_id` (string, e.g. PMID for PubMed, NCT ID for ClinicalTrials, slug for scraped pages), `raw_metadata` (object, source-specific fields).
...
3. Invoke the source module via Bash with `python -m` against its discover entrypoint, passing seeds/queries/filters from `source_config` and the scope constraints. Stream candidates into memory or to a temp file under `./pipeline/discovered/.tmp/`.
